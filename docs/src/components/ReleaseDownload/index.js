...
function Downloads({browsePath, downloads}) {
  ...
  return (
    <ul className={styles.downloadFiles}>
        ...
        <li key={catIdx}>
          ...
          <ul>
            {groupedDownloads[key].map((download, downloadIdx) => (
              <li key={downloadIdx}>
                <a key={downloadIdx} href={repoUrl(`/archive/${browsePath}.${download.file_type}`)}>
      ...
      <li>
        <a href={repoUrl(`/releases`)} target="_blank">browse all files&hellip;</a>
      </li>
    </ul>
  );
}
  ...
  return (
    <div className={styles.downloadTable}>
      <div>
        <div>Version</div>
        <div>
          {version} • {date}{releaseNotesPath && <> • <Link to={useBaseUrl(releaseNotesPath)}>release notes</Link></>}
        </div>
      </div>
      <div>
        <div>License</div>
        <div>
          <a href={repoUrl('/blob/master/LICENSE')} target="_blank">MIT</a>
        </div>
      </div>
      <div>
        <div>Downloads</div>
...
function ReleaseDownload({version}) {
  ...
  return (
    <Layout title="Download" description="Download SteamSpeak for your platform.">
