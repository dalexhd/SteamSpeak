    ...
    <li>
      <div className="badges">
        {commit.breaking_change && (
          ...
          >
            <i className="feather icon-alert-triangle" /> breaking
          </Link>
        )}
        {!commit.breaking_change && commit.highlight_permalink && (
          ...
          >
            <i className="feather icon-gift" /> highlight
          </Link>
        )}
        {commit.pr_number && (
          <a
            href={`https://github.com/dalexhd/SteamSpeak/pull/${commit.pr_number}`}
            target="_blank"
        ...
        {!commit.pr_number && (
          <a
            href={`https://github.com/dalexhd/SteamSpeak/commit/${commit.sha}`}
            target="_blank"
            rel="noopener noreferrer"
            className="badge badge--secondary"
            style={{ minWidth: '65px', textAlign: 'center', }}
...
function Changelog({ version }) {
  ...
  if (onlyversion) {
    baseCommits = baseCommits.filter(
      (commit) => commit.version === onlyversion
    );
  }
