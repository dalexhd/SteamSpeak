...
class Release
      ...
      if commit.breaking_change?
        if !@highlights.any? { |h| h.type?("breaking change") && h.pr_numbers.include?(commit.pr_number) }
          ...
          raise ArgumentError.new(
            ...
            Make the following changes in your `SteamSpeak.toml` file:

            ```diff title="SteamSpeak.toml"
             [sinks.example]
               type = "example"
            -  remove = "me"
            +  add = "me"
  ...
  def compare_url
    @compare_url ||= "https://github.com/dalexhd/SteamSpeak/compare/v#{last_version}...v#{version}"
  end
